...
const ContentUsuariosActivos: React.FC = () => {
  ...
  useEffect(() => {
    if (usuarios) {
      const filteredUsuarios = usuarios.filter(
        (usuario: UserType) =>
          usuario.isActive === true && usuario.isDeleted === false
      );
      setusuariosActivos(filteredUsuarios);
    }
  }, [usuarios]);
  ...
  const sortColumn = (descriptor: SortDescriptor) => {
    if (descriptor.column && descriptor.direction !== undefined) {
      let sortedData = [...usuariosActivos];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Table.Body items={usuariosActivos}>
          {(item: UserType) => (
